<?php
// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared

function servnp() { 
    $serverName = $_SERVER['SERVER_NAME'];
    $serverName = trim($serverName);
    return $serverName;
}

function bpc_all_pro_details_supplied() {
    if (isset($_REQUEST['be_popia_compliant_nonce']) && wp_verify_nonce($_REQUEST['be_popia_compliant_nonce'], 'be_popia_compliant_active_check_nonce')) {

        $url = wp_http_validate_url("https://py.bepopiacompliant.co.za/api/domaincompletecheck/" . esc_html(servnp()));
        $args = array('headers' => array('Content-Type' => 'application/json',),'body' => array(),);
        $response = wp_remote_get(wp_http_validate_url($url), $args);
        $response_code = wp_remote_retrieve_response_code($response);
        $body = wp_remote_retrieve_body($response);
        if (401 === $response_code) {
            echo "Unauthorized access";
        }
        if (200 === $response_code) {
            $body = json_decode($body);
            foreach ($body as $data) {
                $domain_form_complete = $data->domain_form_complete;
                $consent_form_complete = $data->consent_form_complete;
                $other_parties = $data->other_parties;

                global $wpdb;
                $table_name = 'be_popia_compliant_admin';
                if ($domain_form_complete == 1 && $consent_form_complete == 1 && $other_parties != null) {
                    // @phpcsSuppress WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> DELIBERATELY DID NOT CACHE */ $wpdb->update($wpdb->prefix.$table_name, array('value' => 1), array('id' => 4));
                    return 1;
                } else {
                    // @phpcsSuppress WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> DELIBERATELY DID NOT CACHE */ $wpdb->update($wpdb->prefix.$table_name, array('value' => 0), array('id' => 4));
                    return 0;
                }
            }
        }
    }
}


function be_popia_compliant_active_check() {
    // if (isset($_REQUEST['be_popia_compliant_nonce']) && wp_verify_nonce($_REQUEST['be_popia_compliant_nonce'], 'be_popia_compliant_active_check_nonce')) {
        global $wpdb;
        $url = "https://py.bepopiacompliant.co.za/api/domain/check_expiry/" . esc_html(servnp());
        $args = array('headers' => array('Content-Type' => 'application/json',), 'body' => array(),);
        $response = wp_remote_get($url, $args);
        $response_code = wp_remote_retrieve_response_code($response);
        $body = wp_remote_retrieve_body($response);
        if (401 === $response_code) {
            echo "Unauthorized access, You do not seem to be authorised to access this data!";
        }
        if (200 === $response_code) {
            $trim_brackets = trim($body, "[{}]");
            $explode = explode(',', $trim_brackets);
            $trim_date = str_replace('"renew_date":', '', $explode[1]);
            $trim_date = str_replace('"', '', $trim_date);
            $go_on = str_replace('"is_subscribed":', '', $explode[2]);
            $consent_form_complete = str_replace('"consent_form_complete":', '', $explode[3]);
            $domain_form_complete = str_replace('"domain_form_complete":', '', $explode[4]);
            $domain_form_complete = str_replace('"', '', $domain_form_complete);
            $other_parties = str_replace('"other_parties":', '', $explode[5]);
            $other_parties = str_replace('"', '', $other_parties);
            $trim_date = trim($trim_date, '"');
            $go_on = trim($go_on, '"');
            $date = strtotime($trim_date);
            $date = "" . gmdate("Y-m-d", $date) ;
            $now = "" . gmdate("Y-m-d");
            if (($date >= $now) && $consent_form_complete == 1 && $domain_form_complete == 1 && ($other_parties !== null && $other_parties !== '')) {
                if ($go_on == 1) {
                    global $wpdb;
                    $privacy = '';
                    $table_name = 'be_popia_compliant_admin';
                    $privacy_value = wp_cache_get('privacy_value', 'bpc_plugin_cache');
                    if (false === $privacy_value) {
                        // @phpcsSuppress WordPress.DB.DirectDatabaseQuery.DirectQuery
                        /* FALSE-POSITIVE -> */ $wpdb->update($wpdb->prefix.$table_name, array('value' => 0), array('id' => 3));
                        $privacy_value = 0;
                        wp_cache_set('privacy_value', $privacy_value, 'bpc_plugin_cache');
                    }
                    $privacy = $privacy_value;  
                    $banner_background = get_option('be_popia_compliant_banner-field11-background-color');
                    $banner_text = get_option('be_popia_compliant_banner-field12-text-color');
                    echo '<style>
                            .BePopiaCompliant {
                                background-color:' . esc_attr($banner_background) . ';
                                color:' . esc_attr($banner_text) . ';
                                text-align: center;
                                box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);
                            }
                            .cont1 {
                                margin: auto;
                                width: 90%;
                                height: 80px;
                                display: flex;
                            }
                            .be_popia_compliant_img {
                                margin: 20px auto auto 10%;
                                width: 150px;
                            }
                            span.bpc_contnents {
                                padding: 2%;
                            }
                            .be_popia_compliant_links {
                                margin: auto auto auto 0;
                                width: -webkit-fill-available;
                                font-weight:500;
                                font-size: 20px;
                            }
                            .be_popia_compliant_links a {
                                color:' . esc_attr($banner_text) . ';
                                text-decoration: none;
                                font-variant-caps: all-petite-caps;
                                font-weight: 500;
                            }
                            @media only screen and (max-width: 748px) {    
                                .be_popia_compliant_img {
                                    margin: auto auto auto auto;
                                    padding: 15px;
                                }
                                .be_popia_compliant_links {
                                    margin: auto auto auto auto;
                                    width: 100%;
                                    font-weight: 700;
                                    font-size: 23px;
                                }
                                .cont1 {
                                    margin: auto;
                                    width: 80%;
                                    height: 245px;
                                    display: block;  
                                }
                            }
                        </style>
                        <div class="BePopiaCompliant">
                            <div class="cont1">
                                <div class="be_popia_compliant_img">';
                                    if(strval(get_option('be_popia_compliant_cookie-field10-banner-logo-selector')) == 'white') {
                                    echo '<a href="https://bepopiacompliant.co.za" target="_blank"><img alt="POPIA Compliant" src="data:image/png;base64," oncontextmenu="return false;" />
                                    </a>';
                                } elseif(strval(get_option('be_popia_compliant_cookie-field10-banner-logo-selector')) == 'black') {
                                    echo '<a href="https://bepopiacompliant.co.za" target="_blank"><img alt="POPIA Compliant" src="data:image/png;base64," oncontextmenu="return false;" />
                                    </a>';
                                } elseif(strval(get_option('be_popia_compliant_cookie-field10-banner-logo-selector')) == 'grey') {
                                    echo '<a href="https://bepopiacompliant.co.za" target="_blank"><img alt="POPIA Compliant" src="data:image/png;base64," oncontextmenu="return false;" />
                                    </a>';
                                } else {  echo '<a href="https://bepopiacompliant.co.za" target="_blank"><img alt="POPIA Compliant" src="data:image/png;base64," oncontextmenu="return false;" />
                                </a>';
                                }

                                echo '</div>
                                <div class="be_popia_compliant_links">
                                        ';
                    echo '<center>
                    <span class="bpc_contnents"><a href="' . esc_url('https://bepopiacompliant.co.za/#/privacy/' . esc_html(servn())) . '" target="_blank"><span style="white-space:nowrap">PRIVACY POLICY</span></a></span>
                    <span class="bpc_contnents"><a href="' . esc_url('https://manageconsent.co.za/#/main/request/' . esc_html(servn())) . '" target="_blank"><span style="white-space:nowrap">MANAGE CONSENT</span></a></span>
                    <span class="bpc_contnents"><a href="' . esc_url('https://bepopiacompliant.co.za/#/details/' . esc_html(servn())) . '" target="_blank"><span style="white-space:nowrap">RESPONSIBLE PARTIES</span></a></span>
                    <span class="bpc_contnents"><a href="' . esc_url('https://bepopiacompliant.co.za/#/regulator/' . esc_html(servn()))  . '" target="_blank"><span style="white-space:nowrap">INFORMATION REGULATOR</span></a></span></center>';
                    echo '</div>
                    <span style="font-size:0px; position:absolute;">';
                    update_option('bpc_report', '6');
                    echo "BPC REPORT 6: " . esc_html(get_option("bpc_v"));
                    $has_active_keys = get_option('has_active_keys');
                    echo " PRO ";
                    if (get_option("cron_last_fired_at")) {
                        echo esc_html(gmdate("d/m/Y H:i:s", get_option("cron_last_fired_at") + 7200));
                    } else {
                        echo "No Run";
                    }
                    if (get_option("be_popia_compliant_cookie-field9-disable-bpc-cookie-banner") != 1) {
                        echo " Active ";
                    } else {
                        echo " Deactivated ";
                    }
                    if (is_ssl()) {
                        echo "Has SSL";
                    } else {
                        echo "No SSL";
                    }
                    echo '</span>
                            </div>
                        </div>';
                }

            } elseif (($date <= $now) && $consent_form_complete == 1 && $domain_form_complete == 1 && ($other_parties !== null && $other_parties !== '')) {
                global $wpdb;
                $table_name = 'be_popia_compliant_checklist';
                // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $needComms = $wpdb->get_var($wpdb->prepare("SELECT does_comply FROM {$wpdb->prefix}{$table_name} WHERE id = %d", 2));
                // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $needMarketing = $wpdb->get_var($wpdb->prepare("SELECT does_comply FROM {$wpdb->prefix}{$table_name} WHERE id = %d", 3));
                
                if ($needComms == 1 && $needMarketing == 0) {
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query1 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name} WHERE (type < %d AND type > %d) AND does_comply = %d AND (id != %d) AND (id != %d) AND is_active = %d", 8, 0, 1, 3, 59, 1); // Placeholder for 'type' upper limit  // Placeholder for 'type' lower limit // Placeholder for 'does_comply' // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'is_active' 
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results1 = $wpdb->get_results($query1);
                    $rowcount = count($results1);
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query2 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name} WHERE (type < %d AND type > %d) AND (id != %d) AND (id != %d) AND is_active = %d", 8, 0, 3, 59, 1); // Placeholder for 'type' upper limit // Placeholder for 'type' lower limit // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results2 = $wpdb->get_results($query2);
                    $rowcount2 = count($results2);
                    
                } elseif ($needComms == 0 && $needMarketing == 1) {
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query1 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name} WHERE (type < %d AND type > %d) AND does_comply = %d AND (id != %d) AND (id != %d) AND is_active = %d", 8, 0, 1, 2, 58, 1); // Placeholder for 'type' upper limit // Placeholder for 'type' lower limit // Placeholder for 'does_comply' // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results1 = $wpdb->get_results($query1);
                    $rowcount = count($results1);
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query2 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name} WHERE (type < %d AND type > %d) AND (id != %d) AND (id != %d) AND is_active = %d",8, 0, 2, 58, 1); // Placeholder for 'type' upper limit // Placeholder for 'type' lower limit // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results2 = $wpdb->get_results($query2);
                    $rowcount2 = count($results2);
                    
                } elseif ($needComms == 1 && $needMarketing == 1) {
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query1 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name} WHERE (type < %d AND type > %d) AND does_comply = %d AND is_active = %d", 8, 0, 1, 1); // Placeholder for 'type' upper limit // Placeholder for 'type' lower limit // Placeholder for 'does_comply' // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results1 = $wpdb->get_results($query1);
                    $rowcount = count($results1);
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query2 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name} WHERE (type < %d AND type > %d) AND is_active = %d", 8, 0, 1); // Placeholder for 'type' upper limit // Placeholder for 'type' lower limit // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results2 = $wpdb->get_results($query2);
                    $rowcount2 = count($results2);
                    
                } elseif ($needMarketing == 0 && $needComms == 0) {
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query1 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name}  WHERE (type < %d AND type > %d) AND does_comply = %d AND (id != %d) AND (id != %d) AND (id != %d) AND (id != %d) AND is_active = %d", 8, 0, 1, 2, 3, 58, 59, 1); // Placeholder for 'type' upper limitm // Placeholder for 'type' lower limit // Placeholder for 'does_comply' // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results1 = $wpdb->get_results($query1);
                    $rowcount = count($results1);
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query2 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name} WHERE (type < %d AND type > %d) AND (id != %d) AND (id != %d) AND (id != %d) AND (id != %d) AND is_active = %d", 8, 0, 2, 3, 58, 59, 1); // Placeholder for 'type' upper limit // Placeholder for 'type' lower limit // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results2 = $wpdb->get_results($query2);
                    $rowcount2 = count($results2);
                }
                --$rowcount2;
                $rowcounttotal = ($rowcount / $rowcount2) * 100;
                if ($rowcounttotal == 100) {

                    $url = wp_http_validate_url("https://py.bepopiacompliant.co.za/api/plugindetailscheck/" . esc_html(servn()));
                    $args = array(
                        'headers' => array(
                            'Content-Type' => 'application/json',
                        ),
                        'body' => array(),
                    );
                    $response = wp_remote_get(wp_http_validate_url($url), $args);
                    $response_code = wp_remote_retrieve_response_code($response);
                    $body         = wp_remote_retrieve_body($response);

                    if (401 === $response_code) {
                        echo "Unauthorized access";
                    }

                    if (200 === $response_code) {
                        $body = json_decode($body);
                        if ($body != []) {
                            foreach ($body as $data) {
                                $is_approved = $data->is_approved;
                                if ($is_approved) {
                                    $table_name = 'be_popia_compliant_checklist';
                                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $privacy = $wpdb->get_var($wpdb->prepare("SELECT content FROM {$wpdb->prefix}{$table_name} WHERE id = %d", 6));
                                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $data = $wpdb->get_var($wpdb->prepare("SELECT content FROM {$wpdb->prefix}{$table_name} WHERE id = %d", 21));
                                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $parties = $wpdb->get_var($wpdb->prepare("SELECT content FROM {$wpdb->prefix}{$table_name} WHERE id = %d", 32));
                                    echo '<style>
                                        .BePopiaCompliant {
                                            background-color: whitesmoke;
                                            color: #000;
                                            text-align: center;
                                            box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);
                                        }
                                        .cont1 {
                                            margin: auto;
                                            width: 50%;
                                            height: 125px;
                                            display: flex;
                                        }

                                        .bpc_contnents {
                                            margin:0 20px;
                                        }

                                        .be_popia_compliant_img {
                                            margin: auto 0 auto auto;
                                        }
                                        .be_popia_compliant_links {
                                            margin: auto auto auto 0;
                                            width: 75%;
                                            padding: 1%;
                                            font-weight:900;
                                            font-size: 23px;
                                        }
                                        .be_popia_compliant_links a {
                                            color: #BD2E2E;
                                            text-decoration: none;
                                            font-variant-caps: all-petite-caps;
                                        }
                                        @media only screen and (max-width: 748px) { 
                                            
                                            .BePopiaCompliant {
                                                height: 300px;
                                            }

                                            .be_popia_compliant_img {
                                                margin: auto 0 auto auto;
                                            }
                                            .be_popia_compliant_links {
                                                margin: auto auto auto 0;
                                                width: 100%;
                                                font-weight: 900;
                                                font-size: 23px;
                                            }
                                            .cont1 {
                                                margin: auto;
                                                width: 50%;
                                                height: 245px;
                                                display: block;
                                            }
                                        }
                                    </style>
                                    <div class="BePopiaCompliant">
                                        <div class="cont1">
                                            <div class="be_popia_compliant_img">
                                                <a href="https://bepopiacompliant.co.za" target="_blank"><img alt="Self Audited - POPIA Compliant" src="data:image/png;base64," oncontextmenu="return false;" />
                                                </a>
                                            </div>
                                            <div class="be_popia_compliant_links"> <!--
                                                <a href="' . esc_url($privacy) . '" target="_blank"><span style="white-space:nowrap">PRIVACY POLICY</span></a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
                                                <a href="' . esc_url($data) . '"target="_blank"><span style="white-space:nowrap">DATA REQUESTS</span></a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
                                                <a href="' . esc_url($parties) . '" target="_blank"><span style="white-space:nowrap">RESPONSIBLE PARTIES</span></a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
                                                <a href="https://bepopiacompliant.co.za/#/regulator/' . esc_html(servn()) . '" target="_blank"><span style="white-space:nowrap">INFORMATION REGULATOR</span></a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                           -->
                                            <span class="bpc_contnents"><a href="' . esc_url('https://bepopiacompliant.co.za/#/privacy/' . esc_html(servn())) . '" target="_blank"><span style="white-space:nowrap">PRIVACY POLICY</span></a></span>
                                            <span class="bpc_contnents"><a href="' . esc_url('https://manageconsent.co.za/#/main/request/' . esc_html(servn())) . '" target="_blank"><span style="white-space:nowrap">MANAGE CONSENT</span></a></span>
                                            <span class="bpc_contnents"><a href="' . esc_url('https://bepopiacompliant.co.za/#/details/' . esc_html(servn())) . '" target="_blank"><span style="white-space:nowrap">RESPONSIBLE PARTIES</span></a></span>
                                            <span class="bpc_contnents"><a href="' . esc_url('https://bepopiacompliant.co.za/#/regulator/' . esc_html(servn()))  . '" target="_blank"><span style="white-space:nowrap">INFORMATION REGULATOR</span></a></span></center>';
                                           echo'
                                           </div>
                                            <span style="font-size:0px; position:absolute;">';
                                    update_option('bpc_report', '8');
                                    echo "BPC REPORT 8: " . esc_html(get_option("bpc_v"));
                                    $has_active_keys = get_option('has_active_keys');
                                    echo " PRO ";
                                    if (get_option("cron_last_fired_at")) {
                                        echo esc_html(gmdate("d/m/Y H:i:s", get_option("cron_last_fired_at") + 7200));
                                    } else {
                                        echo "No Run";
                                    }
                                    if (get_option("be_popia_compliant_cookie-field9-disable-bpc-cookie-banner") != 1) {
                                        echo " Active ";
                                    } else {
                                        echo " Deactivated ";
                                    }
                                    if (is_ssl()) {
                                        echo "Has SSL";
                                    } else {
                                        echo "No SSL";
                                    }
                                    echo '</span>
                                        </div>
                                    </div>';
                                }
                            }
                        }
                    }
                }
            } else {
                global $wpdb;
                $table_name = 'be_popia_compliant_checklist';
                // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $needComms = $wpdb->get_var($wpdb->prepare("SELECT does_comply FROM {$wpdb->prefix}{$table_name} WHERE id = %d", 2));
                // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $needMarketing = $wpdb->get_var($wpdb->prepare("SELECT does_comply FROM {$wpdb->prefix}{$table_name} WHERE id = %d", 3));
                if ($needComms == 1 && $needMarketing == 0) {
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query1 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name} WHERE (type < %d AND type > %d) AND does_comply = %d AND (id != %d) AND (id != %d) AND is_active = %d", 8, 0, 1, 3, 59, 1); // Placeholder for 'type' upper limit  // Placeholder for 'type' lower limit // Placeholder for 'does_comply' // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results1 = $wpdb->get_results($query1);
                    $rowcount = count($results1);
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query2 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name} WHERE (type < %d AND type > %d) AND (id != %d) AND (id != %d) AND is_active = %d", 8, 0, 3, 59, 1); // Placeholder for 'type' upper limit // Placeholder for 'type' lower limit // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results2 = $wpdb->get_results($query2);
                    $rowcount2 = count($results2);
                    
                } elseif ($needComms == 0 && $needMarketing == 1) {
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query1 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name} WHERE (type < %d AND type > %d) AND does_comply = %d AND (id != %d) AND (id != %d) AND is_active = %d", 8, 0, 1, 2, 58, 1); // Placeholder for 'type' upper limit // Placeholder for 'type' lower limit // Placeholder for 'does_comply' // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results1 = $wpdb->get_results($query1);
                    $rowcount = count($results1);
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query2 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name} WHERE (type < %d AND type > %d) AND (id != %d) AND (id != %d) AND is_active = %d",8, 0, 2, 58, 1); // Placeholder for 'type' upper limit // Placeholder for 'type' lower limit // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results2 = $wpdb->get_results($query2);
                    $rowcount2 = count($results2);
                    
                } elseif ($needComms == 1 && $needMarketing == 1) {
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query1 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name} WHERE (type < %d AND type > %d) AND does_comply = %d AND is_active = %d", 8, 0, 1, 1); // Placeholder for 'type' upper limit // Placeholder for 'type' lower limit // Placeholder for 'does_comply' // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results1 = $wpdb->get_results($query1);
                    $rowcount = count($results1);
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query2 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name} WHERE (type < %d AND type > %d) AND is_active = %d", 8, 0, 1); // Placeholder for 'type' upper limit // Placeholder for 'type' lower limit // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results2 = $wpdb->get_results($query2);
                    $rowcount2 = count($results2);
                    
                } elseif ($needMarketing == 0 && $needComms == 0) {
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query1 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name}  WHERE (type < %d AND type > %d) AND does_comply = %d AND (id != %d) AND (id != %d) AND (id != %d) AND (id != %d) AND is_active = %d", 8, 0, 1, 2, 3, 58, 59, 1); // Placeholder for 'type' upper limitm // Placeholder for 'type' lower limit // Placeholder for 'does_comply' // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results1 = $wpdb->get_results($query1);
                    $rowcount = count($results1);
                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $query2 = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}{$table_name} WHERE (type < %d AND type > %d) AND (id != %d) AND (id != %d) AND (id != %d) AND (id != %d) AND is_active = %d", 8, 0, 2, 3, 58, 59, 1); // Placeholder for 'type' upper limit // Placeholder for 'type' lower limit // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'id' to exclude // Placeholder for 'is_active'
                    // @phpcsSuppress WordPress.DB.PreparedSQL.NotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                    /* FALSE-POSITIVE -> USED PLACEHOLDERS AND WAS PREPARED IN PREVIOUS LINE */ $results2 = $wpdb->get_results($query2);
                    $rowcount2 = count($results2);
                }
                --$rowcount2;
                $rowcounttotal = ($rowcount / $rowcount2) * 100;
                if ($rowcounttotal == 100) {

                    $url = wp_http_validate_url("https://py.bepopiacompliant.co.za/api/plugindetailscheck/" . esc_html(servn()));
                    $args = array(
                        'headers' => array(
                            'Content-Type' => 'application/json',
                        ),
                        'body' => array(),
                    );
                    $response = wp_remote_get(wp_http_validate_url($url), $args);
                    $response_code = wp_remote_retrieve_response_code($response);
                    $body         = wp_remote_retrieve_body($response);

                    if (401 === $response_code) {
                        echo "Unauthorized access";
                    }

                    if (200 === $response_code) {
                        $body = json_decode($body);
                        if ($body != []) {
                            foreach ($body as $data) {
                                $is_approved = $data->is_approved;
                                if ($is_approved) {
                                    $table_name = 'be_popia_compliant_checklist';
                                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $privacy = $wpdb->get_var($wpdb->prepare("SELECT content FROM {$wpdb->prefix}{$table_name} WHERE id = %d", 6));
                                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $data = $wpdb->get_var($wpdb->prepare("SELECT content FROM {$wpdb->prefix}{$table_name} WHERE id = %d", 21));
                                    // @phpcsSuppress WordPress.DB.PreparedSQL.InterpolatedNotPrepared | WordPress.DB.DirectDatabaseQuery.DirectQuery
                                    /* FALSE-POSITIVE -> PLACEHOLDER IS USED FOR VARIABLE, IF USED FOR TABLE NAME THE CODE BREAKS */ $parties = $wpdb->get_var($wpdb->prepare("SELECT content FROM {$wpdb->prefix}{$table_name} WHERE id = %d", 32));
                                    echo '<style>
                                        .BePopiaCompliant {
                                            background-color: whitesmoke;
                                            color: #000;
                                            text-align: center;
                                            box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);
                                        }
                                        .cont1 {
                                            margin: auto;
                                            width: 50%;
                                            height: 125px;
                                            display: flex;
                                        }
                                        .be_popia_compliant_img {
                                            margin: auto 0 auto auto;
                                        }
                                        .be_popia_compliant_links {
                                            margin: auto auto auto 0;
                                            width: 75%;
                                            padding: 1%;
                                            font-weight:900;
                                            font-size: 23px;
                                        }
                                        .be_popia_compliant_links a {
                                            color: #BD2E2E;
                                            text-decoration: none;
                                            font-variant-caps: all-petite-caps;
                                        }
                                        @media only screen and (max-width: 748px) {    
                                            .be_popia_compliant_img {
                                                margin: auto 0 auto auto;
                                            }
                                            .be_popia_compliant_links {
                                                margin: auto auto auto 0;
                                                width: 100%;
                                                font-weight: 900;
                                                font-size: 23px;
                                            }
                                            .cont1 {
                                                margin: auto;
                                                width: 50%;
                                                height: 245px;
                                                display: block;
                                            }
                                        }
                                    </style>
                                    <div class="BePopiaCompliant">
                                        <div class="cont1">
                                            <div class="be_popia_compliant_img">
                                                <a href="https://bepopiacompliant.co.za" target="_blank"><img alt="Self Audited - POPIA Compliant" src="data:image/png;base64," oncontextmenu="return false;" />
                                                </a>
                                            </div>
                                            <div class="be_popia_compliant_links">
                                                <a href="' . esc_url($privacy) . '" target="_blank"><span style="white-space:nowrap">PRIVACY POLICY</span></a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="' . esc_url($data) . '"target="_blank"><span style="white-space:nowrap">DATA REQUESTS</span></a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="' . esc_url($parties) . '" target="_blank"><span style="white-space:nowrap">RESPONSIBLE PARTIES</span></a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="https://bepopiacompliant.co.za/#/regulator/' . esc_html(servn()) . '" target="_blank"><span style="white-space:nowrap">INFORMATION REGULATOR</span></a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                            </div>
                                            <span style="font-size:0px; position:absolute;">';
                                    update_option('bpc_report', '9');
                                    echo "BPC REPORT 9: " . esc_html(get_option("bpc_v"));
                                    $has_active_keys = get_option('has_active_keys');
                                    echo " PRO ";
                                    if (get_option("cron_last_fired_at")) {
                                        echo esc_html(gmdate("d/m/Y H:i:s", get_option("cron_last_fired_at") + 7200));
                                    } else {
                                        echo "No Run";
                                    }
                                    if (get_option("be_popia_compliant_cookie-field9-disable-bpc-cookie-banner") != 1) {
                                        echo " Active ";
                                    } else {
                                        echo " Deactivated ";
                                    }
                                    if (is_ssl()) {
                                        echo "Has SSL";
                                    } else {
                                        echo "No SSL";
                                    }
                                    echo '</span>
                                        </div>
                                    </div>';
                                }
                            }
                        }
                    }
                }
            }
        }
    // }
}

be_popia_compliant_active_check();